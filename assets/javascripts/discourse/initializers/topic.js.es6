...
export default {
  ...
  initialize: function() {
    ...
    var refreshRetorts = function(retorts) {
      var id = parseInt(_.last(this.channel.split('/'))) // sorry mom.
    ...
    Discourse.TopicRoute.on("setupTopicController", function(event) {
      Discourse.ajax('/retorts/index?topic_id=' + event.controller.model.id, refreshRetorts)
